            ...
            @foreach ($accounts as $account)
                <div class="card-dash">
                    <div class="flex items-center justify-between">
                        <h2 class="font-bold text-slate-900">{{ $account->domain_name ?? 'Waiting for your domain' }}</h2>
                        <x-status-badge :status="$account->status" />
                    </div>
                    @if (in_array($account->status, [\App\Enums\HostingStatus::Pending, \App\Enums\HostingStatus::Creating, \App\Enums\HostingStatus::AwaitingDomain, \App\Enums\HostingStatus::Failed, \App\Enums\HostingStatus::ManualReview], true))
                    ...
                    @if ($account->status === \App\Enums\HostingStatus::AwaitingDomain)
                        <a href="{{ route('customer.hosting.show', $account) }}" class="btn-primary btn-sm mt-4">Add your domain</a>
                    @else
