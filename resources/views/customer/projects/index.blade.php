...
@section('content')
    @if ($projects->isEmpty())
        <div class="card text-center">
            ...
            <p class="mt-1 text-slate-500">Order the {{ money_compact($websitePackagePrice ?? config('billing.website_package.price')) }} website package to start a project.</p>
            ...
            @foreach ($projects as $project)
                <a href="{{ route('customer.projects.show', $project) }}" class="card-dash transition hover:shadow-soft">
                    ...
                    <dl class="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-4">
                        ...
                        <div><dt class="text-slate-500">Price paid</dt><dd class="font-medium">{{ $project->order ? money($project->order->total, $project->order->currency, true) : '—' }}</dd></div>
